...
2. **Async, and you have a _window_ of keys to read/write at once?** Use [`session.get_many()` / `session.put_many()`](#async-window-api) — the window API, ~**376K TPS** (~2.7× single-op async, the highest-throughput async mode).
...
**Throughput.** A single async loop is *submission-bound* — the loop thread caps out around **~139K TPS** (fast path) no matter how much in-flight concurrency you add. The window API breaks past that ceiling by amortizing submission across the window: at 50/50 read/write on a 3-node cluster it reaches **~376K TPS at a 16-key window** — the sweet spot — **~2.7×** the single-op path at equal (512) in-flight. A window of 8 gives ~332K at the lowest tail (p99 ~1.4ms); 32+ trade throughput for tail latency (p99 ~12ms). Notably, a single loop running the window API (~376K) out-throughputs the multi-loop `AsyncPool` even at its ~317K peak — it's the highest-throughput async mode. It is **async-only by design**: the sync path has no event loop and already pays minimal per-op overhead, so there is no `get_many`/`put_many` on `SyncSession`.
