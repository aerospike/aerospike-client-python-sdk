...
def _first_active_node_supports_server_compiled_ael_blocking(pac: AsyncClient) -> bool:
    ...
    clients never call async-only PAC APIs (which raise at runtime). Returns
    ``False`` when the PAC client has no node-listing surface (for example the
    experimental per-thread ``_LocalClient`` proxy).
    """
