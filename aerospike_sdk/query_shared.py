...
class _QueryBuilderBase:
    ...
    def order_by(
        ...
        Must be paired with :meth:`top_k`. Validation occurs when the query
        executes.

        Requires Aerospike Server 8.1.3+. The client uses Top-K pushdown when
        supported by all queried nodes and always merges the final result.
        To rank vector similarity, project a vector-distance expression with
        :meth:`with_op_projection` and order by its output bin. Generic
        expression reads of VECTOR bins are not supported.
