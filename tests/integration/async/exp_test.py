...
class TestAelErrorHandling:
    ...
    @requires_client_side_ael
    async def test_invalid_ael_syntax_client_parse(self, client_with_cdt_data):
        """Invalid AEL raises :class:`AelParseException` when parsed client-side."""
        with pytest.raises(AelParseException):
            await (
                client_with_cdt_data.query("test", "cdt_test")
    ...
    @requires_server_compiled_ael
    async def test_invalid_ael_syntax_server_path(self, client_with_cdt_data):
        """Invalid AEL on server path surfaces as ``PARAMETER_ERROR`` from the server."""
        await assert_dataset_invalid_ael_rejected(
            client_with_cdt_data.query("test", "cdt_test")
            .where("this is not valid AEL !!!")
    ...
    @requires_client_side_ael
    async def test_invalid_list_syntax_client_parse(self, client_with_cdt_data):
    ...
    @requires_server_compiled_ael
    async def test_invalid_list_syntax_server_path(self, client_with_cdt_data):
