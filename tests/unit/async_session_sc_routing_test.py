...
If a future change drops ``policy_sc=`` from this call, users on SC
namespaces silently fall back to AP semantics — no exception, no warning,
wrong consistency guarantees during partition / failover. These tests
...
async def test_session_get_ap_and_sc_policies_are_distinct():
    ...
    but they're not. Confirm Behavior.DEFAULT yields distinct caches so the
    routing fix has something meaningful to switch between."""
